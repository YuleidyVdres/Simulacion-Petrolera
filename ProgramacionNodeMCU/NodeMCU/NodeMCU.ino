...
void handleRoot() {
  ...
  <script type=\"text/javascript\">\
    ...
    function conectar()\ /*funcion que se llama al selecionar la red a conectarse*/
    {\
...
void conectar()
{
  ...
  // conectar a la red wifi que escogi en la pagina
  WiFi.begin(red,pw);
...
String calcularSenal(long rssi) // funcion que me calcula la intensidad de la se;al de las redes que tengo cerca 
{
  if(rssi<=-80)
    return "mala";
  else if(rssi<-70)
...
void redes() // imprimir en la pagina con que intensidad esta cada red 
{
...
void setup() {
  ...
  if(cont==20){ // mensaje de error de conexion pasado los 10 seg 
    estado="desconectado";
    WiFi.disconnect();
    Serial.println("error de conexion");  
  }
...
void loop() {
    ...
    {
      Serial.println("loop disconnect"); // mensajes de desconexion 
      WiFi.disconnect();
      estado="desconectado";
      coneccion=false;
    }
